    ...
    return (
      <div className={`flex justify-between items-center p-3 ${bgColor} rounded-lg`}>
        ...
        <div className="text-sm text-muted-foreground">
          {group.primaryGuest.phone_number && (
            <span className="mr-2">{group.primaryGuest.phone_number}</span>
          )}
          <span>{new Date(group.primaryGuest.date_added).toLocaleDateString()}</span>
        </div>
      </div>
    );
  }
          ...
          {group.guests.slice(1).map((guest) => (
            <div key={guest.id} className={`flex justify-between items-center p-2 ${bgColor} rounded border-l-2 border-primary/30`}>
              ...
              <div className="text-xs text-muted-foreground">
                {guest.phone_number && (
                  <span className="mr-2">{guest.phone_number}</span>
                )}
                <span>{new Date(guest.date_added).toLocaleDateString()}</span>
              </div>
            </div>
          ))}
        </div>
