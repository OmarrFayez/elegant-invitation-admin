...
import { X, VolumeX, Volume2, Heart, Calendar, MapPin, Clock, Users, Banknote } from 'lucide-react';
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-auto">
        <Card className="relative overflow-hidden">
          ...
          <CardContent className="relative z-10 p-8 text-center">
            ...
            {/* Payment Information - OMT, Bank, Wish Money - After Descriptions */}
            {(wedding.omt || wedding.bank || wedding.wish_account || wedding.whish_account) && (
              <div className="grid md:grid-cols-3 gap-6 mb-8">
                {wedding.omt && (
                  <div className="text-center p-6 bg-white/90 rounded-lg shadow-lg">
                    <h3 className="text-primary font-semibold mb-4">OMT</h3>
                    <p className="text-foreground">{wedding.omt}</p>
                  </div>
                )}
                
                {wedding.bank && (
                  <div className="text-center p-6 bg-white/90 rounded-lg shadow-lg">
                    <h3 className="text-primary font-semibold mb-4">Bank</h3>
                    <p className="text-foreground">{wedding.bank}</p>
                  </div>
                )}
                
                {(wedding.wish_account || wedding.whish_account) && (
                  <div className="text-center p-6 bg-white/90 rounded-lg shadow-lg">
                    <h3 className="text-primary font-semibold mb-4">Whish</h3>
                    <p className="text-foreground">{wedding.wish_account || wedding.whish_account}</p>
                  </div>
                )}
              </div>
            )}

            {/* Additional Wedding Details - Time and Church Info */}
            {(wedding.time || wedding.church_time || wedding.church_location || wedding.church_name) && (
              <div className="grid md:grid-cols-3 gap-6 mb-8">
                {wedding.time && (
                  <div className="text-center p-6 bg-white/90 rounded-lg shadow-lg">
                    <Clock className="h-6 w-6 text-primary mx-auto mb-4" />
                    <p className="text-foreground">{wedding.time}</p>
                  </div>
                )}
                ...
                {wedding.church_time && (
                  <div className="text-center p-6 bg-white/90 rounded-lg shadow-lg">
                    <Clock className="h-6 w-6 text-primary mx-auto mb-4" />
                    <p className="text-foreground">{wedding.church_time}</p>
                  </div>
                )}
                ...
                {wedding.church_location && (
                  <div className="text-center p-6 bg-white/90 rounded-lg shadow-lg">
                    ...
                    <p className="text-foreground">{wedding.church_location}</p>
                  </div>
                )}
                ...
                {wedding.church_name && (
                  <div className="text-center p-6 bg-white/90 rounded-lg shadow-lg">
                    ...
                    <p className="text-foreground">{wedding.church_name}</p>
                  </div>
                )}
              </div>
            )}
